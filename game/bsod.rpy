...
## 本功能仅限 DDLC Mod 使用。
##
## 可以制造支持全平台的假 BSOD / 内核错误画面。
## （移动端默认采用 Linux 的内核错误画面）
##
## 参数：
##     bsodCode - 显示的错误代码内容（就像 Windows 8 / 8.1 / 10 / 11 那样）。
##                未指定时，默认显示 DDLC_ESCAPE_PLAN_FAILED 错误代码。
##     bsodFile（Windows 7 或 Linux 平台）- 随意定义一个导致 BSOD 的驱动 / 库文件。
##                未指定文件名时默认显示 libGLESv2.dll。
##     rsod （Windows 11）- 强行将 Windows 11 的黑屏死机画面改为红屏。
...
init python:
    ...
    def fakePercent(st, at, winver, chinese_screen):
...
screen bsod(bsodCode="DDLC_ESCAPE_PLAN_FAILED", bsodFile="libGLESv2.dll", rsod=False, chinese_screen=True):
        ...
        if osName < "6.2.9200": # Windows 7
            ...
            if chinese_screen:
                vbox:
                    ...
                    text "您的电脑遇到问题，需要重新启动。" style "bsod_win8_sub_text_cn"
                    text "我们只收集某些错误信息，然后" style "bsod_win8_sub_text_cn"
                    add DynamicDisplayable(fakePercent, 8, chinese_screen=True)
                    text "如果你想了解更多信息，则可以稍后在线搜索此错误：" + bsodCode.upper() style "bsod_win8_sub_text_cn"
                ...
                vbox:
                    ...
                    add DynamicDisplayable(fakePercent, 8, chinese_screen=False)
                    text "If you'd like to know more, you can search online later for this error: " + bsodCode.upper() style "bsod_win8_sub_text_cn"
                    ...
                    hbox:
                        ...
                        if osName >= "10.0.10240" and osName < "10.0.22000":
                            ...
                            vbox:
                                xpos 0.03
                                spacing 4
                                text "有关此问题的详细信息和可能的解决方法，请访问 https://windows.com/stopcode" style "bsod_win10_info_text_cn" line_leading 25
                                text "如果致电支持人员，请向他们提供以下信息：" style "bsod_win10_sub_text_cn" line_leading 25
                                text "终止代码: " + bsodCode.upper() style "bsod_win10_sub_text_cn"
                            ...
                            vbox:
                                xpos 0.03
                                spacing 4
                                text "有关此问题的详细信息和可能的解决方法，请访问" style "bsod_win10_info_text_cn" line_leading 25
                                text "https://windows.com/stopcode\n" style "bsod_win10_info_text_cn"
                                text "如果致电支持人员，请向他们提供以下信息：" style "bsod_win10_sub_text_cn" 
                                text "终止代码: " + bsodCode.upper() style "bsod_win10_sub_text_cn"
                else:
                    hbox:
                        ...
                        if osName >= "10.0.10240" and osName < "10.0.22000":
                            ...
                            vbox:
                                xpos 0.03
                                spacing 4
                                text "For more information about this issue and possible fixes, visit https://windows.com/stopcode" style "bsod_win10_info_text" line_leading 25
...
if renpy.windows:
    ...
    style bsod_win8_text_cn is gui_text
    style bsod_win8_text_cn:
    ...
    style bsod_win8_sad_text_cn is gui_text
    style bsod_win8_sad_text_cn is bsod_win8_text_cn:
        size 128
        xpos -8

    style bsod_win8_sub_text_cn is gui_text
    style bsod_win8_sub_text_cn is bsod_win8_text_cn:
        size 11
    ...
    style bsod_win10_text_cn is bsod_win8_text_cn
    style bsod_win10_text_cn:
        font "C:/Windows/Fonts/msyh.ttc"
        size 24
        line_leading 3
        line_spacing 0
    ...
    style bsod_win10_info_text_cn is bsod_win10_text_cn
    style bsod_win10_info_text_cn:
        size 16

    style bsod_win10_sad_text_cn is bsod_win10_text_cn
    style bsod_win10_sad_text_cn:
        size 136
        xpos -8
